...
//        const ImGuiStyle& style = g.Style;
        ...
        if(hovered)
        {
            ...
            if (p1d < (1 / 10.0)) sel = left;
            if (p2d < (1 / 10.0)) sel = left + 1;
            ...
            if (io.MouseDown[0])
            {
                ...
                if (sel != -1) {
                    ...
                    if( points[sel].x - points[sel + 1].x > -0.03f)
                        points[sel].x -= 0.01f;
                    if( points[sel].x - points[sel - 1].x > 0.03f)
                        points[sel].x += 0.01f;
                }
//
                ...
                points[0].y = std::clamp(points[0].y, 0.0f, 1.0f);
                points[max - 1].y = std::clamp(points[max- 1].y, 0.0f, 1.0f);
            }
...
//        }
        ...
        for (i = 0; i < max; i++)
        {
            ...
            window->DrawList->AddRectFilled(a, b, GetColorU32(ImGuiCol_SliderGrab), 2.0f);
            window->DrawList->AddRect(a, b, ImColor(1.0f, 1.0f, 1.0f, 1.0f), 2.0f);
        }
