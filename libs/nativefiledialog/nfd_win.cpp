...
static nfdresult_t AddFiltersToDialog(::IFileDialog *fileOpenDialog, int images_only )
{

    COMDLG_FILTERSPEC specList[1];
    if (images_only){
        specList[0].pszName = L"Images";
        specList[0].pszSpec = L"*.png;*.jpg;*.jpeg;*.bmp";
    }else{
        specList[0].pszName = L"All";
        specList[0].pszSpec = L"*.*";
    }

    fileOpenDialog->SetFileTypes( 1, specList );
...
nfdresult_t NFD_OpenDialog(nfdchar_t **outPath, const nfdchar_t *defaultPath, int images_only)
{
    ...
    if ( !AddFiltersToDialog(fileOpenDialog, images_only ) )
    {
        goto end;
    }
                                    ...
                                    nfdpathset_t *outPaths )
{
    ...
    // Build the filter list
    if ( !AddFiltersToDialog( fileOpenDialog, 0 ) )
    {
        goto end;
    }
                            ...
                            nfdchar_t **outPath )
{
    ...
    // Build the filter list
    if ( !AddFiltersToDialog( fileSaveDialog, 0 ) )
    {
        goto end;
    }
